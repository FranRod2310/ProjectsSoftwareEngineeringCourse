...
`...\core\src\mindustry\entities\units\AIController`
...
## Class Diagram
![Class Diagram Template Pattern.png](ClassDiagram_TemplatePattern.png)

## Rationale
The **Template Method Pattern** is used to define the general structure of how all AIs update their behaviour.
The method `updateUnit()` in `AIController` acts as the template, defining the algorithm steps: update visuals, targeting, and movement.
Subclasses like `MinerAI`, `BuilderAI`, and `RepairAI` implement the specific details of these steps, adapting the behavior to each unit type.
Although `AIController` is not declared as *abstract*, it functions as one conceptually, the default implementation allows the game to assign a neutral controller when no specific behavior is needed.

### **Benefits** 
- **Code reuse** : Common logic centralized in `AIController`.
- **Consistency**: All unit AIs follow the same update cycle.
- **Extensibility**: New AI types only need to override specific methods.
- **Safety**: Default implementation avoids null or unassigned AI errors.
