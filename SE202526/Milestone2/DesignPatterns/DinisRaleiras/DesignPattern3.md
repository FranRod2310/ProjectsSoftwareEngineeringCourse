...
## Class Diagram
![GameState Class Diagram](ClassDiagram_FacadePattern.png)

## Rationale

The `GameState` class acts as a *facade* (Facade Pattern) because it centralizes and simplifies access to the game’s global state.
Instead of having code interact directly with multiple complex classes like `Rules`, `Teams`, `Map`, or `GameStats, everything goes through `GameState`.

This makes the code:
- Easier to use (centralized access)
- Cleaner (fewer dependencies)
- Easier to maintain (low coupling between modules)
