...
Here:
All members are declared as `static`, meaning there is **only one instance** of each.
The `init()` method acts like the **lazy initializer**, creating these single instances when the game starts.

### Class Diagram
![Factory Method Pattern Diagram](designPattern2UML.png)

### Rationale
The `Vars` class centralizes global state and acts as a **Singleton container**, ensuring:
- There is **only one instance** of each subsystem (e.g., world, UI, network).
- All classes across the engine can access shared data consistently.
- Initialization happens in a controlled manner (`Vars.init()`).

### Benefits
- **Global access**: Systems like AI, rendering, and logic can access game state easily.
- **Consistency**: Prevents accidental creation of multiple conflicting worlds or networks.
- **Simplicity**: Simplifies initialization and debugging by providing one source of truth.