...
In *Mindustry*: `PowerTurret` (or any turret) does not implement how shooting works internally. 
Instead, it delegates the shooting behavior to a `BulletType` object.
...
### Class Diagram
![Factory Method Pattern Diagram](designPattern3UML.png)

`BulletType` serves as a base class for different projectile behaviors. 
Subclasses like `FireBulletType` and `ContinuousBulletType` extend it to implement unique shooting strategies. 
This allows flexible and modular design in gameplay mechanics.

## Rationale
The `BulletType` class in *Mindustry* exemplifies the **Strategy pattern** because it encapsulates 
the shooting behavior of a turret into a separate, interchangeable object. By doing so, 
*Mindustry* achieves several key design goals:
- **Separation of concerns**: Turrets handle targeting, while bullets handle movement, damage, and effects.
- **Flexibility**: Different bullet types can be assigned to the same turret at runtime.
- **Reusability**: New bullet behaviors can be added without modifying turret code.
- **Decoupling**: Turrets depend on bullet behavior abstractly, making the design easier to maintain and test.

This design makes projectile behavior interchangeable and dynamically configurable, matching the core idea of the Strategy pattern.