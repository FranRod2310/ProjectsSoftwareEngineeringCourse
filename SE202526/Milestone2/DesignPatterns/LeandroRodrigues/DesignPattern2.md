...
`...\core\src\mindustry\type\Weapon.java`
...
```
The other classes: `...\core\src\mindustry\type\weapons\`
...
The Weapon class and its subclasses (BuildWeapon, MineWeapon, RepairBeamWeapon, PointDefenseWeapon, etc.)implement
the Strategy design pattern. Weapon defines a common interface for all weapon behaviors (update, shoot, draw), while
each subclass provides a different strategy, such as building, mining, repairing, or defending. This pattern allows
units to change their weapon behavior dynamically without altering their own code, promoting flexibility, 
